...
public class AutoBalanceMultiDirectional extends CommandBase {
    ...
    public void execute() {
        ...
        drivetrain.drive(frontBackOutput+feedforwardX, leftRightOutput+feedforwardY, 0);
    }

    @Override
    public boolean isFinished() {
        return Math.max(drivetrain.getPitch(), drivetrain.getRoll()) < 10 && Math.min(drivetrain.getPitch(), drivetrain.getRoll()) > -10; // || (Math.abs(drivetrain.getPitch()) >=13.5 || Math.abs(drivetrain.getRoll()) >= 13.5);
    }
